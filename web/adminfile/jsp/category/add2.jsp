...
<style type="text/css">
  ...
  <body>
    ...
    <form action="<c:url value='/admin/AdminCategoryServlet'/>" method="post" onsubmit="return checkForm()">
    	...
    	分类名称：<input type="text" name="categoryName" id="categoryName"/><br/>
    	一级分类：<select name="parentId" id="parentId">
		<option value="">===选择1级分类===</option>
		<c:forEach items="${parents}" var="parent">
			<option value="${parent.categoryId}" <c:if test="${parent.categoryId eq parentId}">selected="selected"</c:if> >${parent.categoryName}</option>
		</c:forEach>
    	</select><br/>
